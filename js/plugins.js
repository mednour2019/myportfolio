...
(function (c, k, l) {
  function f() {
    return parseInt(e.scrollTop() + e.height()) === parseInt(m.height())
      ? !0
      : !1;
  }
  function g(a) {
    return null === a
      ? 0
      : "string" === typeof a && 0 < c(a).length
      ? c(a).outerHeight()
      : console.log(
          "Section Scroller Error: Wrong fixedNavbar option provided!"
        );
  }
  var e = c(k),
    m = c(l),
    ...
    d = function (a, b) {
      ...
      this.scrollerButton = c(this.options.scrollerButton);
  ...
  d.prototype = {
    ...
    init: function () {
      var a = this;
      this.scrollerButton.on("click", function (b) {
        a.scrollToNextElement(a);
        b.preventDefault();
      });
      this.scrollerButtonRotateClassHandler(this);
      e.scroll(function () {
        a.scrollerButtonRotateClassHandler(a);
      });
    },
    scrollToNextElement: function (a) {
      for (
        var b = 0, d = a.offset + g(a.fixedNavbar);
        b < a.elements.length &&
        Math.round(e.scrollTop()) >=
          Math.round(c(a.elements[b]).offset().top - d);

      )
        b++;
      if (b >= a.elements.length || f()) b = 0;
      a.scrollToElementIndex = b;
      a.options.onScrollStart.call(a);
      c("html, body")
        .stop()
        .animate(
          {
            scrollTop:
              Math.round(c(a.elements[b]).offset().top - d) +
              Math.round(
                Number(c(a.elements[b]).data("scroll-offset"))
                  ? Number(c(a.elements[b]).data("scroll-offset"))
                  : 0
              ),
          },
    ...
    scrollerButtonRotateClassHandler: function (a) {
      var b = a.offset + g(a.fixedNavbar);
      f()
        ? ((b = a.scrollerButton),
          (a = a.scrollerButtonRotateClass),
          b.hasClass(a) || b.addClass(a))
        : Math.round(e.scrollTop()) >=
          Math.round(c(a.elements[a.elements.length - 1]).offset().top - b)
        ? ((b = a.scrollerButton),
          (a = a.scrollerButtonRotateClass),
          b.hasClass(a) || b.addClass(a))
        : ((b = a.scrollerButton),
  ...
  c.fn.sectionScroller = function (a) {
    c.data(this, "plugin_sectionScroller") ||
      c.data(this, "plugin_sectionScroller", new d(this, a));
    return this;
  };
})(jQuery, window, document);

/* ---------------------------------------------
    08. jQuery Easing v1.3
--------------------------------------------- */
/*!
 * jQuery Easing v1.3 - https://github.com/gdsmith/jquery.easing
 *
...
/* ---------------------------------------------
    12. imagesLoaded PACKAGED v4.1.1
--------------------------------------------- */
/*!
...
$(function () {
  $("form#contact_form").submit(function (e) {
    ...
    if ($email.val() == "" || !re.test($email.val())) {
      $email
        .parent()
        .append('<span class="error">Please provide valid Email.</span>');
    ...
    if ($message.val() == "") {
      $message
        .parent()
        .append(
          '<span class="error">Please enter details About Project.</span>'
        );
    ...
    if (!hasError) {
      ...
      $.ajax({
        ...
        .done(function (response) {
          ...
          $("form#contact_form").prepend(
            '<span class="success">Thank you. Your email was sent successfully.</span>'
          );

          // Clear the form.
          $(
            'form input[name="email"], form input[name="name"], form input[name="subject"], form textarea[name="message"]'
          ).val("");
        })
        .fail(function (data) {
